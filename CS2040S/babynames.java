...
public class babynames {
    ...
    static void insert(TrieNode root, String name) {
        ...
        current.word_count++;
        current.lookup.add(name);
        for (char c : name.toCharArray()) {
            int index = c - 'A';
            if (current.letters[index] == null) //if this prefix has not been added
                current.letters[index] = new TrieNode(); //create new node to represent next letter in prefix
            current = current.letters[index];
            current.word_count++;
        }
        current.word_end = true; //once nodes for every letter in the word have been created, mark end of word
    }
