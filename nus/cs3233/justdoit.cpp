...
int main() {
    ...
    auto dnc = [&](auto &&self, int l, int r) -> modint {
        if (l + 1 == r) return a[l] * a[l];

        int m = l + (r - l) / 2, L = m - l, R = r - m;

        vector<long long> ll(L), lr(L);
        ll[L - 1] = lr[L - 1] = a[m - 1];
        for (int i = m - 2, j = L - 2; i >= l; i--, j--) {
            ll[j] = min(ll[j + 1], a[i]);
            lr[j] = max(lr[j + 1], a[i]);
        }

        vector<long long> rl(R), rr(R);
        rl[0] = rr[0] = a[m];
        for (int i = 1; i < R; i++) {
            rl[i] = min(rl[i - 1], a[m + i]);
            rr[i] = max(rr[i - 1], a[m + i]);
        }

        vector<modint> pref_mul(R), pref_l(R), pref_r(R);
        pref_mul[0] = rl[0] * rr[0];
        pref_l[0] = rl[0];
        pref_r[0] = rr[0];
        for (int i = 1; i < R; i++) {
            pref_mul[i] = pref_mul[i - 1] + rl[i] * rr[i];
            pref_l[i] = pref_l[i - 1] + rl[i];
            pref_r[i] = pref_r[i - 1] + rr[i];
        }

        modint sum = 0;
        for (int i = 0; i < L; i++) {
            int li = -1, ri = R, mi, ml = R;
            while (li + 1 < ri) {
                mi = li + (ri - li) / 2;

                if (rl[mi] <= ll[i]) ri = ml = mi;
                else li = mi;
            }
            ...
            while (li + 1 < ri) {
                ...
                if (rr[mi] >= lr[i]) ri = mr = mi;
                else li = mi;
            }

            if (ml < mr) sum += lr[i] * (pref_l[mr - 1] - (ml > 0 ? pref_l[ml - 1] : 0));
            else if (mr < ml) {
                sum += ll[i] * (pref_r[ml - 1] - (mr > 0 ? pref_r[mr - 1] : 0));
                swap(ml, mr);
            }

            if (ml > 0) sum += (__int128) ml * ll[i] * lr[i];
            if (mr < R) sum += pref_mul[R - 1] - (mr > 0 ? pref_mul[mr - 1] : 0);
        }
        return sum + self(self, l, m) + self(self, m, r);
    };
    cout << dnc(dnc, 0, n);
}
