...
int main() {
    ...
    vector<vector<int>> adj_list_is(size), adj_list_both(size);
    vector<unordered_set<int>> direct(size), transitive(size), superclasses(size), subclasses(size);

    for (auto [a, b] : is) {
        adj_list_is[a].emplace_back(b);
        adj_list_both[a].emplace_back(b);
    }

    for (auto [a, b] : has) adj_list_both[a].emplace_back(b);
    
    auto bfs = [&](deque<int> q, vector<vector<int>> &adj_list) {
    ...
    for (int i = 0; i < size; i++) {
        superclasses[i] = bfs({i}, adj_list_is);
    ...
    for (int i = 0; i < size; i++) transitive[i] = bfs({direct[i].begin(), direct[i].end()}, adj_list_both);
