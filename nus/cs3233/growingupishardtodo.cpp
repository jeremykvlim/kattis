...
int main() {
    ...
    vector<int> activities(n), lis(n, 0);
    vector<modint> dp(n + 1, 0);
    dp[0] = 1;
    vector<vector<pair<int, modint>>> day(n + 1);
    ...
    for (int &a : activities) {
        ...
        if (!day[i].empty() && day[i].back().first == a) {
            ...
            dp[i] = day[i].empty() ? 0 : dp[i] - p.second + day[i].back().second;
        }

        dp[i] += l == -1 ? dp[i - 1] : dp[i - 1] - day[i - 1][l].second;
