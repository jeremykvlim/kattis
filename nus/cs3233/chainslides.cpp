...
int main() {
    ...
    for (int l = 0, r = 0, used = 0; r < m; r++) {
        ...
        while (l <= r && used >= k) {
            ...
            FlowNetwork<int, int> fn(2 * n + 2);
            for (int x = 0; x < n; x++) {
                fn.add_arc(x, x + n, out[x] - 1, 0);
                for (int y = 0; y < n; y++)
                    if (count[x][y]) fn.add_arc(x + n, y, count[x][y], -1);
            }

            for (int i = 0; i < 2 * n; i++)
                if (i < n) fn.add_arc(i, 2 * n + 1, 1, 0);
                else fn.add_arc(2 * n, i, 1, 0);
            fn.add_arc(2 * n + 1, 2 * n, 1e9, 0);

            auto [f, c] = fn.max_flow_min_cost(2 * n, 2 * n + 1);
            if (f != n) break;
