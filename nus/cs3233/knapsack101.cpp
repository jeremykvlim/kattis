...
int main() {
    ...
    for (int d = 1; d <= n; d++) {
        ...
        auto dfs = [&](auto &&self, int i, int j, vector<int> &indices, unordered_map<long long, vector<int>> &half, int depth = 0, modint z = 1) {
            if (half.size() >= 5e4) return;

            if (depth == d) {
                half[z.value] = indices;
                return;
            }
        ...
        for (auto [zl, indices] : left) {
            ...
            if (right.count(zr.value)) {
                for (int j = 0; j < n; j++) s[j] = '1';
                for (int k : indices) s[k - 1] = '0';
                for (int k : right[zr.value]) s[k - 1] = '1';
