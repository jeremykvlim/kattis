...
int main() {
    ...
    vector<int> digit_sum(1e3, 0);
    for (int i = 1; i < 1e3; i++) digit_sum[i] = digit_sum[i / 10] + i % 10;

    vector<modint> p10(19, 1);
    for (int i = 1; i < 19; i++) p10[i] = p10[i - 1] * 10;

    vector<vector<vector<tuple<long long, int, modint>>>> memo(19, vector<vector<tuple<long long, int, modint>>>(136, vector<tuple<long long, int, modint>>(163, {0, 0, 0})));
    auto dp = [&](auto &&self, int d, int sod, int x) -> tuple<long long, int, modint> {
        auto &[count, y, sum] = memo[d][sod][x];
        if (!count && !y && !sum) {
            if (d == 3) {
                while (x < 1e3) {
                    count++;
                ...
                for (int digit = 0; digit <= 9; digit++) {
                    auto [c, z, s] = self(self, d - 1, sod + digit, x);
                    count += c;
                    sum += s + c * digit * p10[d - 1];
                    x = z;
                }
                y = x;
            }
    ...
    while (t--) {
        ...
        auto S = [&](auto &&self, int d = 18, int sod = 0, int x = 1) -> modint {
            if (!n) return 0;
            ...
            for (int digit = 0; digit <= 9; digit++) {
                auto [c, y, s] = dp(dp, d - 1, sod + digit, x);
                if (n >= c) {
                    n -= c;
                    sum += s + c * digit * p10[d - 1];
                    x = y;
                    ...
                    c = n;
                    sum += self(self, d - 1, sod + digit, x) + c * digit * p10[d - 1];
                    break;
                }
            }
            return sum;
