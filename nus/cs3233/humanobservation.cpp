...
int main() {
    ...
    vector<Point<long long>> people(n);
    for (auto &[x, y] : people) cin >> x >> y;

    int k;
    cin >> k;

    auto convex_hull = monotone_chain(people);
    pair<long long, Point<long long>> seat{LLONG_MAX, {INT_MAX, INT_MAX}};
    while (k--) {
        Point<long long> s;
        cin >> s.x >> s.y;

        auto dist = 0LL;
        for (auto p : convex_hull) dist = max(dist, squared_dist(s, p));
        seat = min(seat, make_pair(dist, s));
    }
    cout << seat.second.x << " " << seat.second.y;
}
