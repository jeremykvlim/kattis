...
int main() {
    ...
    while (cin >> n && n) {
        ...
        vector<int> odd;
        for (int i = 1; i <= n; i++)
            if (degree[i] & 1) odd.emplace_back(i);
        if (odd.empty()) {
            cout << sum << "\n";
            continue;
        }
        ...
        int k = odd.size(), d = 0;
        for (int i = 0; i < k; i++)
            for (int j = i + 1; j < k; j++) d = max(d, dist[odd[i]][odd[j]]);

        vector<vector<int>> adj_matrix(k + 1, vector<int>(k + 1, 0));
        for (int i = 1; i <= k; i++)
            for (int j = 1; j <= k; j++)
                if (i != j) adj_matrix[i][j] = d + 1 - dist[odd[i - 1]][odd[j - 1]];

        auto [weight, matches, match] = blossom(k, adj_matrix);
        cout << sum + (k / 2) * (d + 1) - weight << "\n";
    }
}
