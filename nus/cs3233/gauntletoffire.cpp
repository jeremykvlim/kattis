...
int main() {
    ...
    for (int _ = 0; _ < m; _++) {
        ...
        adj_list[a - 1].emplace_back(b - 1, l);
        adj_list[b - 1].emplace_back(a - 1, l);
    }
    if (m == n - 1) adj_list[0].emplace_back(0, 0);

    int cycle_start = -1;
    vector<bool> visited(n, false), cycle(n, false);
    vector<int> cycle_vertices;
    vector<long long> cycle_weights;
        ...
        for (auto [u, w] : adj_list[v])
            if (u != prev) {
                if (visited[u]) {
                    cycle_start = u;
                    cycle[v] = true;
                    cycle_vertices.emplace_back(v);
                    cycle_weights.emplace_back(w);
                    return;
                }
                self(self, u, v);
                if (cycle_start == -2) return;
                if (cycle_start != -1) {
                    if (cycle_start == v) cycle_start = -2;
                    cycle[v] = true;
                    cycle_vertices.emplace_back(v);
                    cycle_weights.emplace_back(w);
                    return;
    ...
    int s = cycle_vertices.size();
    vector<int> cycle_indices(n), subtree_size(n, 1);
    vector<modint> subtree_len(n, 0), danger(n, 0);
    for (int i = 0; i < s; i++) {
        auto dfs1 = [&](auto &&self, int v, int prev = -1) -> void {
            ...
            for (auto [u, w] : adj_list[v])
                if (u != prev && !cycle[u]) {
                    ...
                    subtree_len[v] += subtree_len[u] + w * subtree_size[u];
                }
        };
        dfs1(dfs1, cycle_vertices[i]);
        auto dfs2 = [&](auto &&self, int v, int prev = -1, modint len = 0) -> void {
            danger[v] = subtree_len[v] + len;
            for (auto [u, w] : adj_list[v])
                if (u != prev && !cycle[u]) self(self, u, v, len + subtree_len[v] - subtree_len[u] - w * (2 * subtree_size[u] - n));
        };
        dfs2(dfs2, cycle_vertices[i]);
    }

    if (m == n) {
        modint total = 0;
        cycle_vertices.resize(2 * s);
        cycle_weights.resize(2 * s);
        for (int i = 0; i < s; i++) {
            total += subtree_len[cycle_vertices[i]];
            cycle_vertices[i + s] = cycle_vertices[i];
            cycle_weights[i + s] = cycle_weights[i];
        }

        vector<modint> pref_len(2 * s + 1, 0), pref_size(2 * s + 1, 0);
        for (int i = 0; i < 2 * s; i++) {
            if (i) cycle_weights[i] += cycle_weights[i - 1];
            pref_len[i + 1] = pref_len[i] + cycle_weights[i] * subtree_size[cycle_vertices[i]];
            pref_size[i + 1] = pref_size[i] + subtree_size[cycle_vertices[i % s]];
        }

        vector<modint> cycle_len(s);
        for (int i = 0, j = 0; i < s; i++) {
            while (cycle_weights[i + s] + cycle_weights[i] > 2 * cycle_weights[j]) j++;

            cycle_len[i] = cycle_weights[i + s] * (pref_size[i + s] - pref_size[j])
                         - pref_len[i + s] + 2 * pref_len[j] - pref_len[i + 1]
                         - cycle_weights[i] * (pref_size[j] - pref_size[i + 1])
                         + total - subtree_len[cycle_vertices[i]];
        }
        for (int i = 0; i < n; i++) danger[i] += cycle_len[cycle_indices[i]];
    }

    for (auto d : danger) cout << d << " ";
}
