...
int main() {
    ...
    vector<vector<int>> A055290{
        ...
        {0, 0, 0, 0, 1, 2, 4, 8, 14, 23, 36, 54, 78, 110, 150, 201, 264, 341, 433, 544, 674, 827, 1004, 1209},
        {0, 0, 0, 0, 0, 1, 2, 6, 14, 32, 64, 123, 219, 377, 616, 978, 1496, 2236, 3251, 4637, 6475, 8899, 12025, 16036, 21091, 27432, 35271, 44920, 56656, 70873, 87924, 108298, 132438, 160943, 194358, 233408, 278755, 331266},
    };

    vector<vector<modint>> a(S_ + 1);
    for (int i = S; i <= S_; i++) {
        if (i == 1) {
            a[i] = {1};
            continue;
        }
        a[i].resize(A055290[i].size());
    ...
    for (int i = S; i <= S_; i++) {
        ...
        c[i] = berlekamp_massey(a[i]);
        while (!c[i].back()) c[i].pop_back();
        a[i].resize(c[i].size());
    }

    modint days = 0;
    for (int i = S; i <= S_; i++) days += kitamasa(c[i], a[i], C_ - 1) - kitamasa(c[i], a[i], C - 2);
    cout << days;
}
