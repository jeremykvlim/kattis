...
int main() {
    ...
    vector<int> r(n, 0), g(n, 0), x(n, 0);
    for (int i = 0; i < n; i++)
        if (s[i] == 'R') r[i] = 1;
        else if (s[i] == 'G') g[i] = 1;
        else x[i] = 1;

    auto rg = convolve(r, g), rx = convolve(r, x), gx = convolve(g, x), xx = convolve(x, x);
    int ACs = 0;
    for (int i = 0; i < 2 * n - 1; i++) {
        int red = min(gx[i], R), green = min(rx[i], G);
        ACs = max(ACs, rg[i] + red + green + min((xx[i] - (!(i & 1) && s[i / 2] == 'X')) / 2, min(R - red, G - green)));
    }
    cout << ACs;
}
