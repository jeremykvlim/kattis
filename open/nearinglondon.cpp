...
struct FenwickTree {
    ...
    T pref_product(int i) {
        T product = 1;
        for (; i; i &= i - 1) product *= BIT[i];
        return product;
    }

    T range_product_query(int l, int r) {
        if (l >= r) return 1;
        return pref_product(r) / pref_product(l);
    }
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 0; j < pfs[i].size(); j++)
            if (!j || pfs[i][j] != pfs[i][j - 1]) primes.emplace_back(pfs[i][j]);
    }
    ...
    int m = primes.size();
    vector<modint> inv(m);
    for (int i = 0; i < m; i++) inv[i] = modint::inv(primes[i]);

    vector<modint> powers(Q);
    vector<vector<int>> prev(m);
    FenwickTree<modint> fw(n + 1);
    for (int r = 1; r <= n; r++) {
        for (int i = 0, j = 1; i < pfs[r].size(); i = j++) {
            for (; j < pfs[r].size() && pfs[r][i] == pfs[r][j]; j++);
            for (int k = 0, p = lower_bound(primes.begin(), primes.end(), pfs[r][i]) - primes.begin(); k < j - i; k++) {
                if (k == prev[p].size()) prev[p].emplace_back(0);
                fw.update(prev[p][k] + 1, pfs[r][i]);
                if (r < n) fw.update(r + 1, inv[p]);
                prev[p][k] = r;
            }
        }

        for (auto [l, q] : queries[r]) powers[q] = fw.pref_product(l);
    }
