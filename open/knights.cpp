...
int main() {
    ...
    vector<vector<vector<modint>>> dp((1 << (2 * n)) + 1, vector<vector<modint>>(1 << n, vector<modint>(1 << n, 0)));
    dp[0][0][0] = 1;
    vector<modint> a;
            ...
            for (int m2 = 0; m2 < 1 << n; m2++)
                if (dp[i][m1][m2]) {
                    ...
                    for (int m3 = 0; m3 < 1 << n; m3++) {
                        for (int j = 0; j < n; j++)
                            if ((m3 >> j) & 1)
                                if ((j + 2 < n && (m2 >> (j + 2)) & 1) || (j >= 2 && (m2 >> (j - 2)) & 1) ||
                                    (j + 1 < n && (m1 >> (j + 1)) & 1) || (j >= 1 && (m1 >> (j - 1)) & 1)) goto next;
                        
                        dp[i + 1][m2][m3] += dp[i][m1][m2];
                        next:;
                    }
                }
        a.emplace_back(ai);
