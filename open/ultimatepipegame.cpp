...
int main() {
    ...
    while (t--) {
        ...
        int count = 0;
        vector<vector<int>> c_id(m, vector<int>(n, -1)), h_id(m, vector<int>(n, -1)), v_id(m, vector<int>(n, -1));
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++)
                if (grid[i][j] == '.') {
                    c_id[i][j] = count++;
        ...
        BoundedFlowNetwork<int, long long> bfn(count);
        for (int i = 0; i < m; i++)
            for (int j = 0; j < n; j++)
                if (grid[i][j] == '.') {
                    int c = c_id[i][j], h = h_id[i][j], v = v_id[i][j];
                    if (!((i + j) & 1)) {
                        bfn.add_supply(c, 2);
                        bfn.add_arc(c, h, 0, 1, 0);
                        bfn.add_arc(c, h, 0, 1, H[i][j]);
                        bfn.add_arc(c, v, 0, 1, 0);
                        bfn.add_arc(c, v, 0, 1, V[i][j]);
                        if (i && grid[i - 1][j] == '.') bfn.add_arc(v, v_id[i - 1][j], 0, 1, 0);
                        if (i + 1 < m && grid[i + 1][j] == '.') bfn.add_arc(v, v_id[i + 1][j], 0, 1, 0);
                        if (j && grid[i][j - 1] == '.') bfn.add_arc(h, h_id[i][j - 1], 0, 1, 0);
                        if (j + 1 < n && grid[i][j + 1] == '.') bfn.add_arc(h, h_id[i][j + 1], 0, 1, 0);
                    } else {
                        bfn.add_demand(c, 2);
                        bfn.add_arc(h, c, 0, 1, 0);
                        bfn.add_arc(h, c, 0, 1, H[i][j]);
                        bfn.add_arc(v, c, 0, 1, 0);
                        bfn.add_arc(v, c, 0, 1, V[i][j]);
                    }
                }
