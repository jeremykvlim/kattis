...
int main() {
    ...
    vector<vector<modint>> transition(n, vector<modint>(n));
    for (int i = 0; i < n - 1; i++) {
        modint total = 0;
        vector<modint> a(n);
        for (auto &aij : a) {
        ...
        for (int j = 0; j < n; j++) transition[i][j] = a[j] / total;
    }
    transition[n - 1][n - 1] = 1;

    vector<modint> dp(n, 0), a;
    dp[0] = 1;
    while (a.size() < 2 * n) {
        a.emplace_back(dp[n - 1]);

        vector<modint> temp(n);
        for (int i = 0; i < n; i++)
            if (dp[i])
                for (int j = 0; j < n; j++) temp[j] += dp[i] * transition[i][j];
        dp = temp;
    }
    ...
    auto probability = (modint) 19 / 20;
    for (int t = l; t <= l + 9; t++)
        if (kitamasa(c, a, t) == probability) {
            cout << t;
            exit(0);
        }
    cout << -1;
