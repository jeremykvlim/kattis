...
int main() {
    ...
    vector<modint> fact(r + w + 1, 1), fact_inv(r + w + 1, 1);
    auto prepare = [&]() {
        auto inv = fact;
    ...
    modint k = 0;
    for (int i = 0, m = r + w - 1; i <= w && m >= 0; i++, m -= d + 1) k += (i & 1 ? -1 : 1) * binomial_coefficient_mod_p(w, i, MOD, fact, fact_inv) *
                                                                                              binomial_coefficient_mod_p(m, w - 1, MOD, fact, fact_inv);
    cout << k;
}
