...
int main() {
    ...
    vector<modint> fact(a_max + 1, 1), fact_inv(a_max + 1, 1), p2(a_max + 1, 1);
    auto prepare = [&]() {
        auto inv = fact;
    ...
    modint count = 1;
    for (int i = 0; i < N - 1; i++) {
        auto [n, m] = minmax(a[i], a[i + 1]);
        modint c = 0;
        for (int k = 0; k <= n; k++) c += (k & 1 ? -1 : 1) * binomial_coefficient_mod_p(n, k, MOD, fact, fact_inv) * modint::pow(p2[n - k] - 1, m);
        count *= c;
    }
    cout << count;
}
