...
int main() {
    ...
    vector<vector<pair<int, int>>> indices(2001);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            cin >> curr[j];
            ...
            sum += curr[j];
            if (j) {
                int w = curr[j - 1] + curr[j];
                if (indices[w].size() <= 7 * (k - 1)) indices[w].emplace_back(i * n + j - 1, i * n + j);
            }

            if (i) {
                int w = prev[j] + curr[j];
                if (indices[w].size() <= 7 * (k - 1)) indices[w].emplace_back((i - 1) * n + j, i * n + j);
            }
        }
        swap(prev, curr);
    }
    ...
    for (int w = 2e3; ~w && edges.size() <= 7 * (k - 1); w--)
        for (auto [u, v] : indices[w]) {
            edges.push_back({u, v, w});
            vertices.emplace_back(u);
            vertices.emplace_back(v);
            if (edges.size() > 7 * (k - 1)) break;
        }
    sort(vertices.begin(), vertices.end());
    vertices.erase(unique(vertices.begin(), vertices.end()), vertices.end());

    int m = vertices.size();
    BoundedFlowNetwork<int, long long> bfn(m + 2);
    for (int i = 0; i < vertices.size(); i++) {
        int r = vertices[i] / n, c = vertices[i] % n;
        if (!((r + c) & 1)) bfn.add_arc(m, i, 0, 1);
        else bfn.add_arc(i, m + 1, 0, 1);
    }

    for (auto [u, v, w] : edges) {
        int i = lower_bound(vertices.begin(), vertices.end(), u) - vertices.begin(), j = lower_bound(vertices.begin(), vertices.end(), v) - vertices.begin(),
            r = vertices[i] / n, c = vertices[i] % n;
        if ((r + c) & 1) swap(i, j);
        bfn.add_arc(i, j, 0, 1, -w);
    }
    bfn.add_supply(m, k);
    bfn.add_demand(m + 1, k);
    cout << sum + bfn.min_cost_b_flow().first;
}
