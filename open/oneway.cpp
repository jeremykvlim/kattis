...
int main() {
    ...
    FlowNetwork<int, long long> fn(n + 2);
    while (m--) {
        int u, v;
        cin >> u >> v;
        u--;
    ...
    for (int v = 0; v < n; v++) {
        fn.add_arc(n, v, degree[v], 0);
        int s = fn.network[v].size();
        for (int c = 0; c < s; c++) fn.add_arc(v, n + 1, 1, c);
    }
    fn.min_cost_max_flow(n, n + 1);

    int d = 0;
    for (int v = 0; v < n; v++) {
        int sum = 0;
        for (auto &a : fn.network[n + 1])
            if (a.u == v) sum += a.cap;
        d = max(d, sum);
    }
    cout << d;
