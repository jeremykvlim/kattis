...
int main() {
    ...
    vector<int> indegree(n, 0), degree(n, 0);
    BoundedFlowNetwork<int, long long> bfn(n + 1);
    bfn.add_demand(n, m);
    while (m--) {
        int u, v;
        cin >> u >> v;
        ...
        degree[u]++;
        degree[v]++;
        bfn.add_arc(v, u, 0, 1, 0);
    }

    for (int v = 0; v < n; v++) {
        bfn.add_supply(v, indegree[v]);
        for (int c = 0; c < degree[v]; c++) bfn.add_arc(v, n, 0, 1, c);
    }
    bfn.min_cost_b_flow();
    ...
    for (int v = 0; v < n; v++) {
        int sum = 0;
        for (int e = 0; e < bfn.m; e += 2)
            if (bfn.arcs[e].u == v && bfn.arcs[e].v == n) sum += bfn.arcs[e ^ 1].cap;
        d = max(d, sum);
    }
    cout << d;
}
