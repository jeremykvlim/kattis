...
int main() {
    ...
    vector<modint> dp1(1 << 10, 0), dp2(1 << 10, 0);
    for (int x = 1; x <= n; x++) {
        string I;
        cin >> I;
        ...
        for (int i = 0; i < len; i++) digit[i] = I[i] - '0';

        for (int mask = 0; mask < 1 << 10; mask++) {
            vector<vector<array<int, 2>>> memo(len, vector<array<int, 2>>(2, {-1, -1}));
            auto dp = [&](auto &&self, int pos = 0, int bound = 1, int nlz = 0) {
                if (pos == len) return nlz;
                if (~memo[pos][bound][nlz]) return memo[pos][bound][nlz];
                memo[pos][bound][nlz] = 0;
                for (int d = 0; d <= (bound ? digit[pos] : 9); d++)
                    if (!d && !nlz) memo[pos][bound][nlz] += self(self, pos + 1, bound && d == digit[pos], 0);
                    else if ((mask >> d) & 1) memo[pos][bound][nlz] += self(self, pos + 1, bound && d == digit[pos], 1);
                return memo[pos][bound][nlz];
            };
            dp1[mask] = dp(dp);
        }
        ...
        for (int m1 = 1; m1 < 1 << 10; m1++)
            if (dp1[m1])
                for (int m2 = 0; m2 < 1 << 10; m2++)
                    if (!(m1 & m2) && dp2[m2]) dp2[m1 | m2] += dp1[m1] * dp2[m2];

        cout << "Case " << x << ": " << accumulate(dp2.begin(), dp2.end(), (modint) 0) - 1 << "\n";
    }
}
