...
int main() {
    ...
    auto [matches, match] = gabow(m, edges);
    ...
    vector<int> unused;
    for (int u = 1; u <= m; u++)
        if (!used[u]) unused.emplace_back(u);
    for (int i = 0; i < unused.size(); i += 2) final.emplace_back(unused[i], unused[i + 1]);

    vector<int> teams(m);
    for (int i = 0; i < n; i++) tie(teams[i], teams[m - 1 - i]) = final[i];

    vector<vector<pair<int, int>>> rounds(m - 1);
    for (int r = 0; r < m - 1; r++) {
        rounds[r].emplace_back(teams[0], teams[m - 1]);
        for (int i = 1; i < n; i++) rounds[r].emplace_back(teams[i], teams[m - 1 - i]);
        rotate(teams.begin() + 1, teams.end() - 1, teams.end());
    }

    cout << "YES\n" << matches << "\n";
    for (int r = 1; r < m - 1; r++)
        for (auto [x, y] : rounds[r]) cout << x << " " << y << "\n";
    for (auto [x, y] : rounds[0]) cout << x << " " << y << "\n";
}
