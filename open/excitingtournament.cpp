...
int main() {
    ...
    for (auto &[s, g] : players) cin >> s >> g;
    sort(players.begin(), players.end());

    auto compute = [&](bool maximize) -> long long {
        BoundedFlowNetwork<int, long long> bfn(2 * n + 1);
        for (int i = 0; i < n - 1; i++) {
            bfn.add_supply(i, 1);
            for (int j = i + 1; j < n; j++) {
                int s = players[i].first ^ players[j].first;
                bfn.add_arc(i, j + n, 0, 1, maximize ? 1e9 - s : s);
            }
        }
        for (int i = 0; i < n; i++) bfn.add_arc(i + n, 2 * n, 0, players[i].second - (i != n - 1));
        bfn.add_demand(2 * n, n - 1);

        auto [cost, feasible] = bfn.min_cost_b_flow();
        return maximize ? (n - 1) * 1e9 - cost : cost;
    };
    cout << compute(false) << " " << compute(true);
}
