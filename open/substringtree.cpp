...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = i; j < n; j++) {
            ss += s[j];
            if (!indices.count(ss)) {
                substrings.emplace_back(ss);
                indices[ss] = substrings.size() - 1;
            }
        }
    }
    ...
    vector<int> seen(m, -1);
    for (int v = 0, t = 0; v < m; v++, t++)
        for (int i = 0; i < substrings[v].size(); i++) {
            string ss;
            for (int j = i; j < substrings[v].size(); j++) {
                ss += substrings[v][j];
                int u = indices[ss];
                if (seen[u] != t) {
                    seen[u] = t;
    ...
    vector<modint> dp(m, 0), temp(m, 0);
    dp[indices[s]] = 1;
    while (l--) {
        fill(temp.begin(), temp.end(), 0);
        for (int v = 0; v < m; v++)
            if (dp[v])
                for (int u : adj_list[v]) temp[u] += dp[v];
        dp = temp;
    }
