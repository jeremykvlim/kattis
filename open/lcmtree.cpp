...
int main() {
    ...
    vector<int> nodes(n);
    for (int &v : nodes) cin >> v;
    sort(nodes.begin(), nodes.end());

    vector<vector<int>> LCM(n, vector<int>(n));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) LCM[i][j] = lcm(nodes[i], nodes[j]);

    vector<bool> unique(n, false);
    unique[n - 1] = true;
    for (int i = n - 2; ~i; i--)
        if (nodes[i] != nodes[i + 1]) unique[i] = true;

    vector<int> dupes(n, 1);
    for (int i = 0; i < n; i++)
        while (dupes[i] <= i && nodes[i] == nodes[i - dupes[i]]) dupes[i]++;
