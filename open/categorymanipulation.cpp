...
int main() {
    ...
    while (tc--) {
        ...
        string s(n, 'F');
        for (int i = 0; i < n; i++)
            for (int d0 = 0; d0 < 3 && s[i] == 'F'; d0++) {
                int d1 = (d0 + 1) % 3, d2 = (d0 + 2) % 3;
                for (int sgn : {-1, 1}) {
                    vector<vector<double>> A(5, vector<double>(n - 1, 0));
                    vector<double> b{0, 0, 0, 0, -1}, c(n - 1);
                    for (int j = 0, k = 0; j < n; j++) {
                        if (i == j) continue;

                        int x = a[j][d1] - a[i][d1], y = a[j][d2] - a[i][d2], z = a[j][d0] - a[i][d0];
                        A[0][k] = x;
                        A[1][k] = -x;
                        A[2][k] = y;
                        A[3][k] = -y;
                    ...
                    auto [value, solution] = linear_program_solution(A, b, c);
