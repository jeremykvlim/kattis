...
int main() {
    ...
    int hori = 0, verti = 0;
    vector<vector<int>> h(n, vector<int>(n, -1)), v(n, vector<int>(n, -1));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            if (grid[i][j] != 'X') h[i][j] = j && grid[i][j - 1] != 'X' ? h[i][j - 1] : hori++;

    for (int j = 0; j < n; j++)
        for (int i = 0; i < n; i++)
            if (grid[i][j] != 'X') v[i][j] = i && grid[i - 1][j] != 'X' ? v[i - 1][j] : verti++;

    vector<pair<int, int>> edges;
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            if (grid[i][j] == '-') edges.emplace_back(h[i][j], v[i][j]);
    auto [l, r, matches] = hopcroft_karp(hori, verti, edges);
    cout << matches;
}
