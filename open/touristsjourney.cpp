...
int main() {
    ...
    for (int v : vertices)
        for (int u : vt[v]) edges.push_back({indices[v], indices[u], depth[u] - depth[v]});
    for (auto [u, v] : extra) edges.push_back({indices[u], indices[v], 1});

    vector<vector<modint>> dp(k + 1, vector<modint>(2 * edges.size())), plans(k + 1, vector<modint>(V));
    plans[0][indices[1]] = 1;
    for (int i = 0; i <= k; i++)
        for (int e = 0; e < edges.size(); e++) {
            auto [u, v, w] = edges[e];
            if (i + w > k) continue;
            dp[i + w][2 * e] += plans[i][u] - dp[i][2 * e + 1];
            plans[i + w][v] += plans[i][u] - dp[i][2 * e + 1];
            dp[i + w][2 * e + 1] += plans[i][v] - dp[i][2 * e];
            plans[i + w][u] += plans[i][v] - dp[i][2 * e];
        }
    cout << plans[k][indices[n]];
}
