...
int main() {
    ...
    while (cin >> m && m) {
        ...
        bool inf = false;
        for (int i = 0; i < m; i++)
            if ((f[i] == i && f[i + 1] == i + 1) || (f[i] == i + 1 && f[i + 1] == i && !(n & 1)) ||
                (i < m - 1 && f[i] == i + 2 && f[i + 1] == i + 1 && f[i + 2] == i && !(n & 1))) {
                inf = true;
                break;
            }
        if (inf) {
        ...
        for (int x = 0; x < m; x++) intervals[x] = minmax(f[x], f[x + 1]);

        modint::init();
        auto A = I<modint>(m);
        vector<modint> a{m};
        while (a.size() < 2 * m) {
            Matrix<modint> temp(m);
            vector<modint> diff(m + 1);
            for (int i = 0; i < m; i++) {
                ...
                modint sum = 0;
                for (int j = 0; j < m; j++) temp[i][j] = sum += diff[j];
            }
            A = temp;
            a.emplace_back(trace(A));
        }

        auto c = modint::prime_mod ? berlekamp_massey(a) : reeds_sloane(a);
        a.resize(c.size());
        auto solutions = n < c.size() ? a[n] : kitamasa(c, a, n);

        int states = (m + 1) * 9;
        auto encode = [&](int x, int l, int r) {
            return x * 9 + (l + 1) * 3 + (r + 1);
        };

        auto decode = [&](int i) -> array<int, 3> {
            int x = i / 9, rem = i % 9;
            return {x, rem / 3 - 1, rem % 3 - 1};
        };
        ...
        for (int i = 0; i < states; i++) {
            ...
            int sl = x ? sgn(f[x] - f[x - 1]) : 0, sr = x < m ? sgn(f[x + 1] - f[x]) : 0;
            if (l == 1) l *= sl;
            else if (l == -1) l *= sr;
            if (r == 1) r *= sr;
            else if (r == -1) r *= sl;
            next[i] = encode(f[x], l, r);
        }

        JumpTable<int> jt(n, states, next, vector<int>(states, 0));
        for (int x = 0; x <= m; x++) {
            auto [y, l, r] = decode(jt.jump_up(encode(x, !!x, x < m), n).second);
            if (x == y) solutions += (l != 1 && r != 1) - (l == 1 && r == 1);
        }
        cout << solutions << "\n";
    }
}
