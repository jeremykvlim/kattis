...
int main() {
    ...
    vector<array<modint, 3>> dp(3);
    for (int a = -1; a <= 1; a++)
        if (valid(1, a))
            for (int b = -1; b <= 1; b++)
                if (valid(2, b)) dp[a + 1][b + 1] += (a == b + 1) == (h[1] == h[2]);

    auto consistent = [&](int i, int a, int b, int c) -> bool {
        int l = (i - 1) + a, m = i + b, r = (i + 1) + c;
        if (((l == m) != (h[i - 1] == h[i])) || ((m == r) != (h[i] == h[i + 1])) || ((l == r) != (h[i - 1] == h[i + 1]))) return false;
        return true;
    };

    for (int i = 2; i < n; i++) {
        vector<array<modint, 3>> temp(3);
        for (int a = 0; a < 3; a++)
            for (int b = 0; b < 3; b++)
                if (dp[a][b])
                    for (int c = -1; c <= 1; c++)
                        if (valid(i - 1, a - 1) && valid(i, b - 1) && valid(i + 1, c))
                            if (consistent(i, a - 1, b - 1, c)) temp[b][c + 1] += dp[a][b];
        dp = temp;
    }

    modint sum = 0;
    for (int a = 0; a < 3; a++)
        for (int b = 0; b < 3; b++) sum += dp[a][b];

    modint fact = 1;
    for (int i = 2; i <= n - count; i++) fact *= i;
    cout << fact * sum;
}
