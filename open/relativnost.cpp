...
int main() {
    ...
    for (int i = 1; i <= n; i++)
        if (b[i])
            for (int j = c - 1; j; j--) dp[j] += dp[j - 1] * a[i];
    ...
    while (q--) {
        ...
        if (b[p]) {
            bw /= b[p];
            total /= a[p] + 1;
            for (int j = 1; j < c; j++) dp[j] -= dp[j - 1] * a[p];
        } else {
            count--;
            rgb /= a[p];
        }
        ...
        if (b[p]) {
            ...
            bw *= b[p];
            total *= a[p] + 1;
            for (int j = c - 1; j; j--) dp[j] += dp[j - 1] * a[p];
        } else {
            count++;
            rgb *= a[p];
        }
        ...
        auto purchases = total;
        for (int j = 0; j < c - count; j++) purchases -= dp[j];
        cout << purchases * bw * rgb << "\n";
    }
}
