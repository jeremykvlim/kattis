...
int main() {
    ...
    vector<modint> fact(n + 3, 1), fact_inv(n + 3, 1);
    auto prepare = [&]() {
        auto inv = fact;
    ...
    while (q--) {
        ...
        if (t == 1) {
            ...
            nodes.resize(m);
            for (int &ai : nodes) cin >> ai;
            build_virtual_tree(vt, nodes, in, lca);
            ...
            for (int i = nodes.size() - 1; ~i; i--) {
                ...
                for (int u : vt[v]) {
                    ...
                    if (!d) {
                        ...
                        auto f1 = fib[d + 1] * dp1[u] + fib[d] * dp2[u], f2 = fib[d] * dp1[u] + fib[d - 1] * dp2[u];
                        dp1[v] *= f1 + f2;
                        dp2[v] *= f1;
                    }
                }
            }
            cout << dp1[nodes[0]] + dp2[nodes[0]] << "\n";
            for (int ai : nodes) vt[ai].clear();
        }
    }
}
