...
int main() {
    ...
    auto i1 = [&](int v, int d, int i, int j) -> int {
        return v * (__lg(n) + 2) * (k + 2) * 2 + d * (k + 2) * 2 + i * 2 + j;
    };

    auto i2 = [&](int v, int d, int j) -> int {
        return v * (__lg(n) + 2) * 2 + d * 2 + j;
    };

    modint count = n;
    vector<modint> dp1((n + 1) * (__lg(n) + 2) * (k + 2) * 2, 0), dp2((n + 1) * (__lg(n) + 2) * 2, 0);
    auto dfs = [&](auto &&self, int v = 1, int prev = -1) -> void {
        for (int d = 0; d <= __lg(n); d++) dp1[i1(v, d, 0, 0)] = 1;
        ...
        for (int u : adj_list[v])
            if (u != prev) {
                ...
                for (int i = k; ~i; i--)
                    for (int j = 0; j < 2; j++)
                        for (int d = 0; d <= __lg(n); d++) {
                            auto temp = dp1[i1(v, d, i, j)];
                            if (temp) {
                                if (d) dp1[i1(v, d, i + 1, j)] += temp * dp2[i2(u, d - 1, 0)];
                                if (!j) dp1[i1(v, d, i + 1, 1)] += temp * dp2[i2(u, d + 1, 1)];
                            }
                        }
            }

        for (int d = 0; d <= __lg(n); d++) {
            dp2[i2(v, d, 0)] = dp1[i1(v, d, k, 0)];
            dp2[i2(v, d, 1)] = dp1[i1(v, d, k, 1)] + dp1[i1(v, d, k - 1, 0)];
            count += dp1[i1(v, d, k + 1, 1)] + dp1[i1(v, d, k, 0)];
        }
        dp2[i2(v, 0, 0)]++;
        count += dp1[i1(v, 0, 1, 1)];
    };
    dfs(dfs);
    cout << count;
}
