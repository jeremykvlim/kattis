...
int main() {
    ...
    vector<array<int, m>> board(n);
    for (auto &row : board)
        for (int &e : row) cin >> e;

    BoundedFlowNetwork<int, long long> bfn(n * m + 2);
    bfn.add_supply(n * m, k);
    bfn.add_demand(n * m + 1, k);
    for (int r = 0; r < n; r++)
        for (int c = 0; c < m; c++) {
            auto id = [&](int r, int c) {
                return m * r + c;
            ...
            int v = id(r, c);
            if ((r + c) & 1) bfn.add_arc(v, n * m + 1, 0, 1);
            else {
                bfn.add_arc(n * m, v, 0, 1);
                if (r) bfn.add_arc(v, id(r - 1, c), 0, 1, -board[r][c] - board[r - 1][c]);
                if (r + 1 < n) bfn.add_arc(v, id(r + 1, c), 0, 1, -board[r][c] - board[r + 1][c]);
                if (c) bfn.add_arc(v, id(r, c - 1), 0, 1, -board[r][c] - board[r][c - 1]);
                if (c + 1 < m) bfn.add_arc(v, id(r, c + 1), 0, 1, -board[r][c] - board[r][c + 1]);
            }
        }
