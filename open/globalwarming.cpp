...
int main() {
    ...
    vector<vector<long long>> adj_matrix(n + 1, vector<long long>(n + 1, 0));
    while (m--) {
        int p, q, c;
        cin >> p >> q >> c;

        adj_matrix[p][q] = adj_matrix[q][p] = 1e9 - c;
    }

    auto [weight, matches, match] = blossom(n, adj_matrix);
    if (matches != n / 2) cout << "impossible";
    else cout << (long long) 1e9 * (n / 2) - weight;
}
