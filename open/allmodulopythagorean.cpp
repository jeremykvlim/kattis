...
int main() {
    ...
    for (auto i = 1LL; i < n; i++) freq[(i * i) % n]++;

    vector<long long> same(n, 0), diff(n, 0), c = convolve(freq, freq);
    for (int i = 0; i < n; i++) diff[i] = c[i] + c[i + n];

    if (n & 1)
        for (int i = 0; i < n; i++) same[i] = freq[(i * inv(2LL, (long long) n)) % n];
    else
        for (int i = 0; i < n; i += 2) {
            same[i] = freq[(i / 2) % (n / 2)];
            if ((i / 2) % (n / 2) + n / 2 < n) same[i] += freq[(i / 2) % (n / 2) + n / 2];
        }

    auto count = 0LL;
    for (int i = 0; i < n; i++) count += ((diff[i] + same[i]) / 2) * freq[i];
    cout << count;
}
