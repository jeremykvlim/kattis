...
int main() {
    ...
    while (t--) {
        ...
        BoundedFlowNetwork<int, long long> bfn(2 * h * w);
        for (int i = 0; i < h; i++)
            for (int j = 0; j < w; j++)
                if (city[i][j] != '#') {
                    if (city[i][j] == '*') {
                        bfn.add_arc(index(i, j), index(i, j) + 1, 0, 1, -1);
                        bfn.add_arc(index(i, j), index(i, j) + 1, 0, 1, 0);
                    } else bfn.add_arc(index(i, j), index(i, j) + 1, 0, 2, 0);

                    if (i + 1 < h && city[i + 1][j] != '#') bfn.add_arc(index(i, j) + 1, index(i + 1, j), 0, 2, 0);
                    if (j + 1 < w && city[i][j + 1] != '#') bfn.add_arc(index(i, j) + 1, index(i, j + 1), 0, 2, 0);
                }
        bfn.add_supply(index(0, 0), 2);
        bfn.add_demand(index(h - 1, w - 1) + 1, 2);
