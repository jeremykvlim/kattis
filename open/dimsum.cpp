...
int main() {
    ...
    vector<modint> p2(total + 1, 1);
    for (int i = 1; i <= total; i++) p2[i] = p2[i - 1] * 2;

    modint sum = 0;
    int pref = 0, suff = 0;
    for (int i = 1, j = n - 1, k = total - count[0]; i < j; i++, j--) {
        k -= count[i] + count[j];
        sum += i * ((p2[count[i]] - 1) * p2[pref] + (p2[count[j]] - 1) * p2[suff]) + p2[k] * j * ((p2[pref + count[i]] - 1) * (p2[count[j]] - 1) + (p2[suff + count[j]] - 1) * (p2[count[i]] - 1) - (p2[count[i]] - 1) * (p2[count[j]] - 1));
        pref += count[i];
        suff += count[j];
    }

    if (!(n & 1)) sum += (p2[count[n / 2]] - 1) * n / 2 * (p2[pref] + p2[suff] - 1);
    sum *= p2[count[0]];
    cout << sum;
}
