...
int main() {
    ...
    auto prepare = [&]() {
        ...
        for (int i = 1; i <= w + h; i++) fact[i] = i * fact[i - 1];
    };
    prepare();

    auto ways = [&](Point<int> a, Point<int> b) -> modint {
        auto c = b - a;
        return binomial_coefficient_mod_p(c.x + c.y, c.x, MOD, fact, fact_inv);
    };
    ...
    for (int mask = 1; mask < 1 << n; mask++) {
        vector<int> indices;
        for (int i = 0; i < n; i++)
            if ((mask >> i) & 1) indices.emplace_back(i);
