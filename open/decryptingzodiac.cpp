...
constexpr unsigned int MODULO = 167772161;
...
int main() {
    ...
    generate(indices.begin(), indices.end(), [i = 0]() mutable { return 52 * i++; });

    vector<int> a(indices[n]), b(indices[n]);
    for (int i = 0; i < n; i++) {
        char c;
        cin >> c;
    ...
    for (int i = 0; i < n; i++) {
        ...
        b[indices[i] + (c - 'a')] = b[indices[i] + 26 + (c - 'a')] = 1;
    }
    reverse(a.begin(), a.end());

    auto c = convolve(a, b);
    int correct = 0;
    for (int i = 0; 2 * indices[i] < c.size(); i++)
        for (int j = 0; j < 26 && indices[i] + j < c.size(); j++) correct = max(correct, c[indices[i] + j] + c[indices[i] + j + indices[n]]);
    cout << n - correct;
}
