...
int main() {
    ...
    vector<vector<double>> A(6, vector<double>(n, 0));
    for (int i = 0; i < n; i++) {
        cin >> A[0][i];
    ...
    auto p = linear_program_solution(A, vector<double>{x, -x, y, -y, 1, -1}, vector<double>(n, 0)).second;
    if (p.empty()) {
        cout << "No";
        exit(0);
    }
    ...
    cout << "Yes\n";
    for (auto pi : p) cout << fixed << setprecision(8) << pi / sum << " ";
}
