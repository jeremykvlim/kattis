...
int main() {
    ...
    for (int &ai : a) {
        ...
        a_max = max(a_max, ai);
        if (ai <= k) b.emplace_back(ai);
    }
    sort(b.begin(), b.end());

    int m = b.size();
    vector<int> pref(m + 1);
    vector<vector<modint>> dp(m + 1, vector<modint>(k + 2, 0));
    dp[0][1] = 1;
    for (int h = 0; h < m; h++) {
        pref[h + 1] = pref[h] + b[h];
        for (int i = h + 1; i; i--)
            for (int j = min(k, b[h] + pref[h] - pref[h - i + 1]); j >= b[h] + pref[i - 1]; j--) dp[i][j + 1] += dp[i - 1][j - b[h] + 1];
    }
    for (int i = 0; i <= m; i++)
        for (int j = 1; j <= k + 1; j++) dp[i][j] += dp[i][j - 1];
    ...
    vector<modint> coeff(n, 1);
    for (int i = 0; i < n; i++) coeff[i] /= binomial_coefficient_mod_p(n - 1, i, MOD, fact, fact_inv) * n;

    auto deduped = a;
    sort(deduped.begin(), deduped.end());
    deduped.erase(unique(deduped.begin(), deduped.end()), deduped.end());

    vector<modint> p(a_max + 1, 0);
    for (int ai : deduped)
        for (int j = 0; j <= min(n - 1, k / ai); j++) {
            modint sum = 0;
            for (int i = 0, l = max(0, k - (j + 1) * ai + 1), r = k - j * ai; i <= min(m, n - 1 - j); i++) sum += coeff[i + j] * (dp[i][r + 1] - dp[i][l]);

            if (j & 1) p[ai] -= sum;
            else p[ai] += sum;
        }
    for (int ai : a) cout << p[ai] << "\n";
}
