...
int main() {
    ...
    auto a_max = 0LL, total = 0LL;
    for (int i = 0; i < n; i++) {
        long long ai;
        cin >> ai;

        pref.emplace_back(total);
        seen1.emplace(total);
        a_max = max(a_max, ai);
        total += ai;
    }
    auto m = min((long long) n, total / a_max);

    auto divs = divisors(total);
    sort(divs.begin(), divs.end());
    divs.erase(unique(divs.begin(), divs.end()), divs.end());
    divs.erase(divs.begin() + (upper_bound(divs.begin(), divs.end(), m) - divs.begin()), divs.end());

    int bound = 2e3;
    vector<int> candidates;
    unordered_set<long long> seen2;
    for (int d : divs)
        if (d <= bound) {
    ...
    for (int i = max(0, (int) divs.size() - bound); i < divs.size(); i++) {
        int d = divs[i];
        if (d > bound) {
            candidates.emplace_back(d);
            seen2.emplace(d);
    ...
    for (int _ = 0; _ < n; _++) {
        int i = rng() % n, j = rng() % n;
        if (i == j) continue;
        auto d = total / gcd(total, abs(pref[i] - pref[j]));
        if (bound < d && d <= m && seen2.emplace(d).second) candidates.emplace_back(d);
    }
