...
vector<long long> sieve(int n) {
    vector<int> spf(n + 1, 0), primes, pp(n + 1), sum_pp(n + 1);
    vector<long long> sum_divisors(n + 1, 1);
    for (int i = 2; i <= n; i++) {
        if (!spf[i]) {
            spf[i] = pp[i] = i;
            primes.emplace_back(i);
            sum_pp[i] = sum_divisors[i] = i + 1;
        }
        ...
        for (int p : primes) {
            ...
            if (p == spf[i]) {
                pp[j] = pp[i] * p;
                sum_pp[j] = sum_pp[i] + pp[j];
                sum_divisors[j] = sum_divisors[i / pp[i]] * sum_pp[j];
                break;
            } else {
                pp[j] = p;
                sum_pp[j] = p + 1;
                sum_divisors[j] = sum_divisors[i] * sum_pp[j];
            }
        }
    }
    return sum_divisors;
...
int main() {
    ...
    auto sum_divisors = sieve(n);
    modint s = 0, sum = 0, p5 = 1;
    for (int i = 1; i <= n; i++) {
        s += 2 * i - 1 - sum_divisors[i];
        sum += s * p5;
        p5 *= 5;
    }
    cout << sum;
