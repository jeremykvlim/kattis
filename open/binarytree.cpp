...
constexpr unsigned int MODULO = 21092013;
...
int main() {
    ...
    for (int tc = 0; tc < n; tc++) {
        ...
        vector<int> left(t.size()), right(t.size());
        ...
        vector<modint> dp(t.size() + 2);
        dp[t.size()] = 1;
        for (int i = t.size() - 1; ~i; i--)
            dp[i] = t[i] == 'U' ? dp[i + 1] : 2 * dp[i + 1] - dp[(t[i] == 'L' ? left[i] : right[i]) + 1];

        auto nodes = dp[0];
        for (int i = 0; i < t.size() && !st.empty(); i++)
            if (t[i] == 'U') {
                nodes += dp[(st.top() != 'L' ? left[i] : right[i]) + 1] + 1;
                st.pop();
            }

        cout << "Case " << tc + 1 << ": " << nodes << "\n";
    }
}
