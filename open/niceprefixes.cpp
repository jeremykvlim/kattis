...
int main() {
    ...
    vector<modint> dp(k + 1, 0), temp(k + 1);
    dp[k] = 1;
    auto walk = [&](auto &dp, int t) {
        fill(temp.begin(), temp.end(), 0);
        for (int a = 1; a <= k; a++)
            if (dp[a]) {
                int b = (k - ((2 * a + t) % k)) % k;
                if (a > 1) temp[a - 1] += dp[a] * a;
                else temp[b + 1] += dp[a];
                if (b) temp[a] += dp[a] * b;
            }
        dp = temp;
    ...
    vector<vector<modint>> block_dp{dp};
    vector<modint> a{1};
    while (a.size() < 2 * k) {
        for (int t = 0; t < k; t++) walk(dp, t);
        block_dp.emplace_back(dp);
        a.emplace_back(accumulate(dp.begin(), dp.end(), (modint) 0));
    }

    auto c = berlekamp_massey(a);
    vector<modint> block_a;
