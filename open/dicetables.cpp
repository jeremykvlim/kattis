...
int main() {
    ...
    vector<int> dice(5), sides{4, 6, 8, 12, 20};
    for (int i = 0; i < 5; i++) cin >> dice[i];

    vector<vector<modint>> memo(5, vector<modint>(21, -1));
    auto dp = [&](auto &&self, int i = 0, int j = 1) -> modint {
        if (i == 5) return 1;
        if (!dice[i]) return self(self, i + 1, j);
