...
int main() {
    ...
    for (int i = 0; i < m; i++) indices[divs[i]] = i;

    vector<vector<array<int, 3>>> count(m);
    count[0].resize(n);
    for (int i = 0; i < n; i++) count[0][i][s[i] - 'a'] = 1;
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (i) {
            ...
            count[i].resize(d);
            for (int j = 0, l = d * pf; j < d; j++)
                for (int k = j; k < l; k += d)
                    for (int c = 0; c < 3; c++) count[i][j][c] += count[indices[l]][k][c];
        }

        string t(d, 'a');
        int changes = 0, extra = n + 1;
        for (int j = 0; j < d; j++) {
            int c = max_element(count[i][j].begin(), count[i][j].end()) - count[i][j].begin();
            changes += q - count[i][j][c];
            extra = min(extra, count[i][j][c] - max(count[i][j][(c + 1) % 3], count[i][j][(c + 2) % 3]));
            t[j] += c;
        }

        auto pi = prefix_function(t);
        if (pi.back() && !(d % (d - pi.back()))) changes += extra;
        cout << changes << " ";
    }
}
