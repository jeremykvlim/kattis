...
int main() {
    ...
    vector<vector<double>> dist(n, vector<double>(n, 0)), dp(n, vector<double>(n, 0));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
    ...
    for (int i = 0; i < n; i++) {
        auto get = [&](int row, int col) {
            if (col > row) return -1e20 * (2 * (col - row) - 1);
            int j = (i + 1 + col) % n, k = (i + 2 + row) % n;
            return dist[i][j] + dist[j][k];
        };

        int m = n - 2;
        auto rows = smawk<double>(m, m, get, cmp);
        for (int row = 0; row < m; row++) dp[i][(i + 2 + row) % n] = rows[row].first;
    }
    ...
    for (int i = 0; i < n; i++) {
        auto get = [&](int row, int col) {
            if (col > row) return -1e20 * (2 * (col - row) - 1);
            int j = (i + 2 + col) % n, k = (i + 4 + row) % n;
            return dist[i][j] + dist[j][k];
        };

        int m = n - 4;
        auto rows = smawk<double>(m, m, get, cmp);
        for (int row = 0; row < m; row++) dp[i][(i + 4 + row) % n] = rows[row].first;
    }
