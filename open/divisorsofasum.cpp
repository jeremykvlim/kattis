...
template <typename T>
gp_hash_table<T, int, Hash> factorize(T n) {
...
int main() {
    ...
    auto pfs1 = factorize(n), pfs2 = factorize(n + 1);
    for (auto [pf, freq] : pfs2) pfs1[pf] += freq;
    auto d = 1LL;
    for (auto [pf, freq] : pfs1) d *= pf == 2 ? freq : freq + 1;
    cout << d;
}
