...
int main() {
    ...
    vector<pair<int, int>> translators(m);
    for (auto &[l1, l2] : translators) cin >> l1 >> l2;

    vector<pair<int, int>> edges;
    for (int i = 0; i < m; i++)
        for (int j = i + 1; j < m; j++) {
            auto [l1, l2] = translators[i];
            auto [l3, l4] = translators[j];
            if (l1 == l3 || l1 == l4 || l2 == l3 || l2 == l4) edges.emplace_back(i + 1, j + 1);
        }
    ...
    for (int i = 1; i <= m; i++)
        if (i < match[i]) cout << i - 1 << " " << match[i] - 1 << "\n";
}
