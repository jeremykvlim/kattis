...
int main() {
    ...
    vector<int> len(n), prev(n), next(n);
    iota(prev.begin(), prev.end(), -1);
    iota(next.begin(), next.end(), 1);
    for (int i = 0; i < n; i++) {
        int rank = sa.SA_inv[i];
        if (rank) len[i] = lcp[rank - 1];

        int l = prev[rank], r = next[rank];
        if (l >= 0) next[l] = r;
        if (r < n) {
            prev[r] = l;
            len[i] = max(len[i], lcp[r - 1]);
            lcp[r - 1] = min(lcp[r - 1], lcp[rank - 1]);
        }
    }
    ...
    for (int i = n - 1; ~i; i--) {
        dp[i] = dp[i + 1] + 1;
        if (len[i]) dp[i] = min(dp[i], dp[i + len[i]] + 3);
    }
    cout << dp[0];
}
