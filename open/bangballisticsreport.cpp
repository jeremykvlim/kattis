...
int main() {
    ...
    for (int v = 1; v < n; v++) {
        int i = (v + sheriff) % n;
        adj_list[(k[i] - sheriff + n - 1) % n].emplace_back(v);
    }

    int count = 0;
    vector<int> l(n), r(n), subtree_size(n, 1);
    ...
    for (int v = 1; v < n; v++) {
        int m = lower_bound(adj_list[v].begin(), adj_list[v].end(), v) - adj_list[v].begin();
        for (int i = m - 1, s = 0; ~i; i--) f /= s += subtree_size[adj_list[v][i]];
        for (int i = m, s = 0; i < adj_list[v].size(); i++) f /= s += subtree_size[adj_list[v][i]];
    }

    int m = adj_list[0].size();
    for (int i = m - 1, s = 0; i; i--) f /= s += subtree_size[adj_list[0][i]];

    modint sum = 1;
    for (int i = m - 2, s = n - 1 - subtree_size[adj_list[0].back()]; ~i; i--) {
        sum = sum * (n - 1 - s) / s + 1;
        s -= subtree_size[adj_list[0][i]];
    }
    cout << f * sum;
}
