...
int main() {
    ...
    while (m--) {
        ...
        adj_matrix[u - 1][v - 1] = adj_matrix[v - 1][u - 1] = true;
    }

    vector<int> side(n, 0);
    for (int v = 2; v < n; v++)
        if (adj_matrix[0][v]) side[v] = 1;
        else if (adj_matrix[1][v]) side[v] = 2;

    TwoSATSystem sat(n);
    for (int v = 2; v < n; v++)
        if (!side[v]) sat.assign(v);

    for (int u = 2; u < n; u++)
        for (int v = u + 1; v < n; v++)
            if (!adj_matrix[u][v]) {
                sat.add_clause(~u, ~v);
                if (side[u] && side[u] == side[v]) sat.add_clause(u, v);
            }
    ...
    vector<int> arya{1}, sansa{2};
    for (int v = 2; v < n; v++)
        if (!assignment[v]) {
            if (side[v] == 1) arya.emplace_back(v + 1);
            else sansa.emplace_back(v + 1);
        }
