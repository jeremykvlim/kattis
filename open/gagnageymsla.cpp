...
int main() {
    ...
    vector<int> l(size + 1), r(size + 1), offset(size + 1);
    ...
    vector<vector<modint>> dp(n + 1, vector<modint>(size + 1, 0));
    dp[0][0] = dp[0][1] = 1;
    for (int i = 0; i <= n; i++) {
        vector<modint> pref(size + 1, dp[i][0]);
        for (int j = 1; j <= size; j++) pref[j] = pref[j - 1] + dp[i][j];

        for (int s = 2; s <= size; s++) {
            if (l[s] > r[s] || i + offset[s] > n - s) continue;
            dp[i + offset[s]][s] += pref[r[s]];
            if (l[s]) dp[i + offset[s]][s] -= pref[l[s] - 1];
        }
    }
