...
int main() {
    ...
    vector<modint> pk(n + 1, 1);
    for (int i = 1; i <= n; i++) pk[i] = pk[i - 1] * (k - 1);

    auto cycle_colors = [&](int len) -> modint {
        if (len == 1) return k;
        return len & 1 ? pk[len] - k + 1 : pk[len] + k - 1;
    };
    ...
    for (int i = 1; i <= n; i++) {
        ...
        vector<int> seen;
        int v = i;
        for (; !visited[v] && !~indices[v]; v = f[v]) {
            seen.emplace_back(v);
            indices[v] = seen.size() - 1;
        }

        if (~indices[v]) {
            ways *= cycle_colors(seen.size() - indices[v]);
            ways *= pk[indices[v]];
        } else ways *= pk[seen.size()];
