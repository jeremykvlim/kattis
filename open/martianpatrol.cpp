...
int main() {
    ...
    vector<modint> inv(n + 4, 1);
    for (int i = 2; i <= n + 3; i++) inv[i] = (MOD - MOD / i) * inv[MOD % i];

    vector<modint> motzkin(n + 2, 1);
    for (int i = 2; i < n + 2; i++) motzkin[i] = ((2 * i + 1) * motzkin[i - 1] + 3 * (i - 1) * motzkin[i - 2]) * inv[i + 2];
    cout << (n + 1) * motzkin[n + 1];
}
