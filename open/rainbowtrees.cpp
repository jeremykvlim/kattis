...
int main() {
    ...
    for (int X = 1; X <= C; X++) {
        ...
        for (int _ = 0; _ < n - 1; _++) {
            ...
            adj_list[x - 1].emplace_back(y - 1);
            adj_list[y - 1].emplace_back(x - 1);
        }

        auto dfs = [&](auto &&self, int v = 0, int prev = -1, int degree = 0, int depth = 0) -> modint {
            int d = adj_list[v].size() - (prev != -1), c = k - degree - (depth > 1);

            modint cPd = 1;
            for (int i = 0; i < d; i++) cPd *= c - i;

            for (int u : adj_list[v])
                if (u != prev) cPd *= self(self, u, v, d, depth + 1);
            return cPd;
        };
