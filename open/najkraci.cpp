...
int main() {
    ...
    for (int i = 1; i <= m; i++) {
        ...
        adj_list[o].push_back({d, l, i});
    }

    vector<modint> count1(n + 1), count2(n + 1), paths(m + 1, 0);
    vector<int> dist(n + 1);
    vector<bool> visited_v(n + 1), visited_e(m + 1);
    ...
    for (int s = 1; s <= n; s++) {
        ...
        count1[s] = 1;
        fill(visited_v.begin(), visited_v.end(), false);
    ...
    for (int i = 1; i <= m; i++) cout << paths[i] << "\n";
}
