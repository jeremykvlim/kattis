...
int main() {
    ...
    int m = s.size(), k = min(n, 2LL * m);
    vector<vector<modint>> dp(k + 1, vector<modint>(m, 0));
    dp[0][0] = 1;
    for (int i = 0; i < k; i++) {
        dp[i + 1][0] = dp[i][0] * 25;
        for (int j = 1; j < m; j++) dp[i + 1][j] = dp[i][j] * 25 + dp[i][j - 1];
    }

    vector<modint> a(k + 1);
    for (int i = 0; i <= k; i++) a[i] = accumulate(dp[i].begin(), dp[i].end(), (modint) 0);
