...
int main() {
    ...
    vector<vector<int>> cubes(t + 1);
    while (n--) {
        int s, a;
        cin >> s >> a;

        cubes[s].emplace_back(a);
    }

    vector<int> a_abs;
    vector<pair<int, int>> candidates;
    for (int s = 1; s <= t; s++) {
        sort(cubes[s].rbegin(), cubes[s].rend());
        if (cubes[s].size() > t / s) cubes[s].resize(t / s);
        for (int a : cubes[s]) {
            candidates.emplace_back(s, a);
            a_abs.emplace_back(abs(a));
        }
    }
    sort(a_abs.begin(), a_abs.end());
    a_abs.erase(unique(a_abs.begin(), a_abs.end()), a_abs.end());

    int lg = __lg(t) + 2;
    vector<int> pref(a_abs.size());
    for (int i = 1; i < a_abs.size(); i++) pref[i] = pref[i - 1] + min(a_abs[i] - a_abs[i - 1], lg);

    int m = pref.back() + lg;
    vector<dynamic_bitset<>> dp(t + 1, dynamic_bitset<>(m + 1));
    dp[0][m] = true;
    vector<bool> reachable(t + 1, false);
    reachable[0] = true;
    vector<modint> value(t + 1, 0);
    for (auto [s, a] : candidates) {
        int i = lower_bound(a_abs.begin(), a_abs.end(), abs(a)) - a_abs.begin();
        auto p2 = modint::pow(2, a_abs[i]);
        if (a < 0) p2 = -p2;
        for (int j = t; j >= s; j--)
            if (reachable[j - s]) {
                auto temp = dp[j - s];
                ...
                do temp.flip(p);
                while (temp[p++] == (a < 0));
                if (dp[j] < temp || !reachable[j]) {
                    dp[j] = temp;
                    reachable[j] = true;
                    value[j] = value[j - s] + p2;
                }
            }
    }
    cout << value[t];
