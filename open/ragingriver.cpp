...
int main() {
    ...
    while (l--) {
        ...
        check(e1);
        check(e2);
        fn.add_arc(e1, e2, 1, 1);
        fn.add_arc(e2, e1, 1, 1);
    }
    fn.add_arc(r + 2, r, p, 0);

    auto [f, c] = fn.min_cost_max_flow(r + 2, r + 1);
    if (p == f) cout << c;
    else cout << p - f << " people left behind";
}
