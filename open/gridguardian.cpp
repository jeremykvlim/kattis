...
int main() {
    ...
    int size = 1 << n;
    vector<vector<vector<int>>> masks(n + 1, vector<vector<int>>(n + 1, vector<int>(size, 0)));
    for (int i = 0; i <= n; i++)
        for (int j = 0; j <= n; j++)
            for (int mask = 0; mask < size; mask++) {
                ...
                for (int k = 0; k < n; k++)
                    if ((mask >> k) & 1) {
                        if (j <= k && k < i - 1 && !((mask >> (k + 1)) & 1)) masks[i][j][mask] ^= 1 << k;
                        if (i < k && k < j && !((mask >> (k - 1)) & 1)) masks[i][j][mask] ^= 1 << k;
                    }
            }
