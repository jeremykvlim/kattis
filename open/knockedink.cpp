...
template <typename T>
T area_of_union_of_circles(const vector<Circle<T>> &circles) {
    int n = circles.size();
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < n; j++)
            if (i != j) {
                ...
                if (!sgn(d)) {
                    if (circles[i].encloses(circles[j]) && (circles[i].radius > circles[j].radius || j > i)) continue;
                    intervals.clear();
                    break;
                }
                ...
                if (l > r) intervals = add({r, l});
                else {
                    auto temp = add({r, 2 * M_PI});
                    intervals = add({0, l});
...
int main() {
    ...
    vector<double> t(n);
    for (int i = 0; i < n; i++) cin >> coords[i].x >> coords[i].y >> t[i];
    ...
    while (l + 1e-6 < r && l + l * 1e-6 < r) {
        ...
        for (int i = 0; i < n; i++) circles[i] = {coords[i], max(0., m - t[i])};

        if (area_of_union_of_circles(circles) >= a) r = m;
        else l = m;
    }
