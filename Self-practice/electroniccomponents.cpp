...
int main() {
    ...
    vector<vector<pair<int, long long>>> placements(max_element(components.begin(), components.end())->first + 1);
    vector<long long> dp(max_element(components.begin(), components.end())->first + 1, LLONG_MAX);
    dp[0] = 0;
    for (int i = 0; i < n; i++) {
        int f = components[i].first;
        long long t = components[i].second;
