...
int main() {
    ...
    while (c--) {
        ...
        long double time = LDBL_MAX;
        for (int i = 0; i < n; i++)
            for (int j = 0; j < i; j++) {
                long double t;
                auto boat1 = make_pair(position[i].first - position[j].first, position[i].second - position[j].second), v = make_pair(velocity[i].first - velocity[j].first, velocity[i].second - velocity[j].second);
